...
describe("isModelAvailableInServer", () => {
  ...
  test("returns false for a model that is available by default", () => {
    expect(isModelAvailableInServer("", "gpt-5.6", "openai")).toBe(false);
  });

  test("returns true when '-all' disables every model", () => {
    expect(isModelAvailableInServer("-all", "gpt-5.6", "openai")).toBe(true);
  });

  test("returns true when the specific model is disabled", () => {
    expect(
      isModelAvailableInServer("-gpt-5.6@openai", "gpt-5.6", "openai"),
    ).toBe(true);
  });
  ...
  test("a model re-enabled after '-all' is not reported as disabled", () => {
    expect(
      isModelAvailableInServer("-all,+gpt-5.6@openai", "gpt-5.6", "openai"),
    ).toBe(false);
  });
});
